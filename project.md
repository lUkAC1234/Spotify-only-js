...
**Spotify** is a music-streaming web application — a Spotify-style player layered on a multi-provider catalog: Jamendo + Audius (free, legally-redistributable) and an opt-in unofficial Yandex Music provider for popular Russian-language music. It is a class-based React 19 + MobX SPA against a Django 5.2 + DRF + Postgres backend, with a Spotify-minimalism UI (dark theme, green accent, content-first layout).

**Phase 18 — Reactive cooldown + visible busy state:**
- **Restored `ACTION_GUARD_DEFAULT_COOLDOWN_MS = 1000`**: spam protection comes back, but now the cooldown is *visible*. Previously the cooldown lived in a plain `Map<string, number>` checked by `Date.now()`, so MobX never knew time was passing — the spinner disappeared the moment the network request finished and the next click felt frozen for the remaining ~800 ms.
- **Cooldown is now MobX-reactive**: `ActionGuard.cooldownKeys` is an `@observable Set<string>`. `startCooldown(key, ms)` adds the key, schedules a `setTimeout` that removes it, and the timer handle is tracked in `cooldownTimers` so concurrent cooldowns on the same key don't stack. `isBusy(key)` returns `inflightKeys.has(key) || cooldownKeys.has(key)`, so the spinner stays visible across both phases — request in-flight *and* the post-completion 1 s window.
- **New `isCoolingDown(key)` getter** for components that want to distinguish active network calls from anti-spam cooldown (none use it yet, but it's there if a tooltip ever needs it).
- **`reset(key)` / `clear()` clean both observables and timers**: a `clearTimeout` is called on every tracked handle so unmounting a service doesn't leave a stray `setTimeout` callback that mutates an `@action`.
- **UX result**: heart click → heart flips immediately (optimistic) → spinner overlays the icon → network finishes (~200 ms) → spinner *stays* through the 1 s cooldown → spinner disappears, heart is interactive again. No more "I clicked unlike and nothing happened" — the spinner makes the wait state explicit. Same pattern for follow / save / submit / patch buttons everywhere `ActionGuard` is used.
- **Build green**: `tsc --noEmit` 0 errors, `eslint --max-warnings 0` 0 errors, `vite build` 91 PWA entries / ~973 KiB.

**Phase 17 — Spinner/Skeleton loaders + ActionGuard UX fix:**
- **Bug fix in `ActionGuard`**: previous default `cooldownMs = 1000` silently dropped the *second* click on toggle-style actions (heart, follow), so unliking a track within a second of liking it appeared frozen — guard returned `null` before the optimistic toggle ran. Default cooldown is now **0** (pure in-flight dedup); cooldown stays opt-in for actions that genuinely need spam protection (none currently set). Toggling heart/follow now feels instant on every click; only concurrent in-flight requests for the same entity are deduplicated.
- **MobX-observable in-flight tracking**: `ActionGuard.inflightKeys` is now an `@observable` `Set<string>`. `isBusy(key)` / `isInflight(key)` are reactive — components re-render automatically when an action starts and finishes. New helpers exposed on services: `LibraryService.isTrackBusy(id)` / `isAlbumBusy(id)` / `isArtistBusy(id)` / `isPlaylistBusy(id)` / `isCreatingPlaylist`, `SocialService.isFollowBusy(userId)` / `isPrivacyBusy`. UI components read these to render spinners.
- **New `Spinner` component** at `app/shared/ui/loaders/spinner.tsx`: class-based `PureComponent`, sizes `xs (12) | sm (16) | md (20) | lg (28) | xl (40)` driven by new `--spinner-*` tokens in `_layout-tokens.scss`, tones `current | accent | muted`, optional `inline` flag, `role="status"` + `aria-live="polite"`. CSS animation respects `prefers-reduced-motion` (slows to 1800 ms instead of stopping so the user still sees motion).
- **New `Skeleton` component** at `app/shared/ui/loaders/skeleton.tsx`: shapes `block | text | circle`, customizable `width / height / radius`, shimmer animation via background-position keyframes, halts on reduced-motion. Uses new `--skeleton-radius` token. Available for places where a spinner is the wrong fit (covers, hero rows, etc.) — opt-in per component.
- **Wired into mutating UI**: heart button in `track-row` and `bottom-player` swaps the heart SVG for an `sm` Spinner while `library.isTrackBusy(track.id)` is true; album-hero save button swaps for an `md` Spinner; artist-hero follow button swaps the label for an inline `sm` Spinner; user-profile follow button does the same; login / register / settings (profile + password) submit buttons render an inline `sm` Spinner + localized "saving" label; create-playlist modal submit button renders an inline `sm` Spinner + the existing `playlist.saving` label. Every busy button also gets `aria-busy="true"` and a `--busy` modifier (`cursor: progress`) so the visual + assistive states stay in sync.
- **i18n parity**: new `settings.saving` key in en/ru/uz `common.json` for the settings submit-busy label.
- **Net UX result**: every mutation that hits the backend now has visible feedback within ~16 ms of the click — the user sees the spinner spin up, not silence. Optimistic-UI state still flips first (heart fills/empties immediately), the spinner overlays only while the network call is in-flight.
- **Build green**: `tsc --noEmit` 0 errors, `eslint --max-warnings 0` 0 errors, `vite build` 91 PWA entries / ~978 KiB.

**Phase 16 — Anti-spam ActionGuard + redundant-fetch elimination:**
- **New `ActionGuard` primitive** at `app/shared/utils/classes/ActionGuard.ts`: per-key Map of in-flight promises + per-key cooldown timestamps. `run(key, fn, { cooldownMs? })` returns the live promise for parallel calls (in-flight dedup), refuses re-entry within `cooldownMs` after completion (default 1000 ms), exposes `isBusy(key)` / `isInflight(key)` for UI spinners, and per-key `reset(key)` / global `clear()`. One instance per service, granular keys per entity (`library:track:123`, `library:playlist:update:42`, `social:follow:7`, etc.) so unrelated entities never block each other.
- **Auth mutations under guard**: `AuthService.login / register / logout / updateProfile / uploadAvatar / deleteAvatar / changePassword` all run through `ActionGuard` with action-level keys (`auth:login`, `auth:register`, …). `fetchMe()` now de-dupes a parallel-call storm via a single in-flight promise field (`fetchMePromise`) — bootstrap + ProfileMenu + sidebar can call simultaneously and only one network request fires. New `applyMePatch(partial)` action lets callers update the user object after a successful PATCH without a follow-up `GET /auth/me/`.
- **Library mutations under guard**: `toggleTrackSaved / toggleAlbumSaved / toggleArtistFollowed / createPlaylist / updatePlaylist / deletePlaylist / uploadPlaylistCover / removePlaylistCover / addTracksToPlaylist / movePlaylistItem / removePlaylistItem`. Per-entity keys (`library:track:<id>`, `library:playlist:update:<id>`, etc.) so toggling heart on track A doesn't lock track B.
- **Read-side dedup + freshness on `LibraryService`**: `fetchSavedTracks / fetchSavedAlbums / fetchFollowedArtists / fetchMyPlaylists` each track an in-flight promise, last-fetched timestamp, and last-requested limit. Within `FRESH_TTL_MS = 30_000` the cached observable list is returned without hitting the network — *unless* a higher `limit` is requested. Fixes the `/library` (50 items) → `/library/liked` (100 items) double-fetch identified in the audit. `refreshAll()` resets timestamps and force-refreshes (called on auth flip).
- **Social mutations + bootstrap-aware polling**: `SocialService.toggleFollow / patchPrivacy` under guard. The friends-activity reaction now keys on `auth.isAuthenticated && auth.isBootstrapped` so the 30-second poller never fires before `/auth/me/` confirms the session — fixes the "polling fires while session is stale localStorage" race.
- **Settings privacy patch**: `SettingsService.togglePrivacy` no longer calls `auth.fetchMe()` after `social.patchPrivacy()` — instead applies the returned patch via the new `auth.applyMePatch(...)` action. Saves one `GET /auth/me/` per privacy toggle.
- **Search discovery once-only loader**: `SearchService.loadDiscovery()` tracks `hasLoadedGenres` + `hasLoadedRecents` flags and returns early when both are warm. Genres are fetched once per session; recent-searches are re-marked stale on auth-state change so a fresh login pulls them. Eliminates the duplicate `GET /catalog/genres/` + `GET /catalog/recent-searches/` that the audit flagged on every Search-page mount.
- **Recent-search mutations under guard**: `CatalogService.pushRecentSearch / removeRecentSearch / clearRecentSearches` use per-action keys with the search service's `ActionGuard`. Spam-clicking the trash icon on a recent-search chip can no longer fire 3× DELETEs.
- **Playlist mutations: optimistic upsert instead of refetch**: `createPlaylist / updatePlaylist / uploadPlaylistCover / removePlaylistCover` no longer `await fetchMyPlaylists()` after success — they call the new `upsertPlaylistInList(detail)` action that converts the `PlaylistDetail` response into a `PlaylistSummary` and merges it into `myPlaylists`. `addTracksToPlaylist / removePlaylistItem` use `bumpPlaylistTrackCount(id, delta)` for the count delta. Net result: 1 mutation = 1 round-trip, not 2.
- **PlayerSync hydrate guard**: hydrate is now keyed on `auth.isAuthenticated && auth.isBootstrapped` and gated by `isHydrating` + `hasHydrated` flags so a single `GET /playback/state/` fires per session, even if auth flickers during bootstrap.
- **Search debounce defaults updated**: `SEARCH_DEBOUNCE_MS = 220 ms` (text input typing — interactive feel preserved) with `cancel()` on empty-query reset; the `ActionGuard.run()` 1000 ms cooldown applies to mutating actions (button clicks). No 1 s lag on text input or on the first click — only repeat-clicks within the cooldown window are dropped.
- **Build green**: `tsc --noEmit` 0 errors, `eslint --max-warnings 0` 0 errors, `vite build` 87 PWA entries / ~961 KiB. Net redundant-request reduction (estimated by the audit): 6–12 fewer HTTP calls per active session.

**Phase 15 — Multi-source discovery aggregation + richer Audius + broader seed catalog:**
- **Audius is no longer passive in discovery**: `AudiusProvider` now overrides `popular_tracks` (`/v1/tracks/trending?time=week`), `recent_tracks` (`/v1/tracks/trending?time=month`), and `tracks_by_tag` (`/v1/tracks/trending?genre=<tag>`). Previously these inherited the empty `()` default from `MusicProvider`, so the home page silently fell back to Jamendo-only — Audius tracks never appeared in Popular / New Releases / Genres rows even when configured.
- **Round-robin discovery aggregation**: `PopularTracksView`, `NewReleasesView`, and the no-seed branch of `RecommendationsView` now merge tracks from every configured provider (Jamendo if `JAMENDO_CLIENT_ID` is set, Audius always, Yandex if `YANDEX_MUSIC_TOKEN` is set). Implementation: new `_aggregated_discovery_response` helper + `_round_robin_merge` that interleaves tracks 1-by-1 across providers and dedupes on `(source, source_id)`. Per-source failures are isolated — a 5xx from one upstream doesn't break the row, the helper just skips that bucket. Aggregation is the default; `?source=jamendo|audius|yandex` forces single-source. `DEFAULT_CATALOG_SOURCE` env var (when non-empty) opts out of aggregation globally.
- **Expanded Jamendo seed catalog**: `seed_featured_playlists` command grew from 6 → 17 curated playlists. Added Jazz / Classical / World / Reggae / Latin / Ambient / Dance / Metal / Folk plus two Audius-sourced trending playlists (Electronic + Hip-Hop/Rap). New `FeaturedSeed.source` field lets each seed pull from its own upstream. Per-playlist track limit bumped 18 → 24. Run `python manage.py seed_featured_playlists` to rebuild.
- **`.env.example` clarified**: `YANDEX_MUSIC_TOKEN` and `DEFAULT_CATALOG_SOURCE` now have inline comments. `DEFAULT_CATALOG_SOURCE` defaults to empty (= aggregation mode); `YANDEX_MUSIC_TOKEN` is documented as opt-in unofficial provider with the OAuth URL inline.
- **Net effect on home page**: Popular Tracks row goes from "12 indie Jamendo tracks" to "~6 Jamendo + ~6 Audius trending, interleaved" by default — same legal upstreams, ~2× the variety, no new credentials required for the Audius half. Adding Yandex token (still optional, still gated, still ToS-aware) makes it 3-way. Cache keys include the aggregation marker (`catalog:popular:agg:<limit>`) so per-source and aggregated responses don't collide.

**Phase 14 — Search hardening (relevance, caching, dedup):**
- **Backend FTS + trigram fusion**: `search_tracks_local` now combines PostgreSQL FTS (`search_vector` + `websearch_to_tsquery`) with `pg_trgm` trigram similarity over `title`, `artist__name`, `album__title`. Ranking is `(-rank, -similarity, -popularity, title)` so an exact FTS hit always wins, fuzzy hits backfill, and ties break on popularity. `search_artists_local` and `search_albums_local` switched from `icontains`-only to `TrigramSimilarity` + icontains union — typo-tolerant matching ("metalica" ↔ "metallica") at GIN-index speed.
- **New trigram migration `catalog/0004_pg_trgm_search.py`**: `TrigramExtension()` enables `pg_trgm`, plus three GIN trigram indexes (`catalog_track_title_trgm`, `catalog_artist_name_trgm`, `catalog_album_title_trgm` with `gin_trgm_ops`). Run `python manage.py migrate catalog` after pulling. Requires Postgres user with `CREATE EXTENSION` privilege (one-time superuser action — `pg_trgm` ships with Postgres core, no extra package).
- **Query normalization**: new `normalize_search_query()` in `apps.catalog.services` collapses whitespace, strips, and caps at 120 chars. Used everywhere — local search, cache key, weak-match check.
- **Backend response cache**: `CatalogSearchView` caches the full payload in Django's `LocMemCache` for `SEARCH_CACHE_TTL = 60` seconds. Cache key is `catalog:search:<source>:<kinds>:<limit>:<offset>:<blake2s(query)>`. Newly-synced upstream tracks appear in cached searches within the TTL; non-cached queries trigger the same FTS + trigram + upstream-fallback pipeline. View was decomposed into `_build_payload`, `_is_weak_match`, `_sync_upstream`, `_empty_payload`, `_cache_key` for readability.
- **Lower weak-match threshold** (`5 → 4`) and tighter trigger logic — trigram fuzzy matching catches more candidates locally, so the upstream Jamendo/Audius fallback fires less often. The fallback still upserts new entities into Postgres so the next identical query is fully local.
- **Frontend `SearchCacheService`** at `app/core/services/catalog/search-cache.service.ts`: sessionStorage-backed LRU (`MAX_ENTRIES = 64`, TTL = 5 min) keyed by `query|kinds|limit|offset`. Survives in-app navigation, dies on tab close. Hydrates lazily on first read; persists on every mutation; auto-evicts the oldest entry once full; falls back to no-op when `sessionStorage` is unavailable (SSR / locked storage / quota exceeded).
- **`CatalogService.search` rewritten**: cache-first lookup, then in-flight request deduplication (a `Map<cacheKey, Promise>` so two parallel calls for the same query share a single network round-trip), then network with `AbortController` cancellation; result is written back to the cache on success. `loadMoreSearchTracks` reuses the same cache for paginated track requests. New `invalidateSearchCache(query?)` method (full clear or per-query prefix wipe).
- **Smarter debounce**: search debounce dropped from 250 ms → 220 ms with explicit `{ leading: false, trailing: true }`. New `SEARCH_MIN_QUERY_LENGTH` config (currently 1) gates fetching on too-short queries; the debounced fetch is `cancel()`-ed when the URL query is cleared so a stale call never lands after the user wipes the input.
- **Defense-in-depth**: frontend cache hit avoids the network entirely; backend cache hit avoids the Postgres FTS plan; FTS index hit avoids the seq scan; trigram floor (`0.18`) keeps low-quality fuzzy matches out. Net effect: typing "billie e" → "billie ei" → "billie eil" → "billie eilish" issues at most one network call per unique query, and repeats are instant.
- **Why no Redis**: existing infra already uses `LocMemCache` for discovery endpoints; adding Redis means new dep + container + env + docker-compose churn for marginal gain on a single-instance app. The big saving is the *frontend* sessionStorage cache (avoids the network entirely); the backend Django cache is a secondary defense that can be swapped to `django-redis` later via the `CACHES` setting alone.
- **Build green**: `tsc --noEmit` 0 errors, `eslint --max-warnings 0` 0 errors, `vite build` 3.18 s (PWA precache 87 entries / ~950 KiB), `manage.py check` 0 issues, `makemigrations --check catalog` reports no drift.

**Phase 13 — Yandex Music as third provider (unofficial, opt-in):**
- New `apps.catalog.providers.yandex_music_unofficial.YandexMusicProvider` (`source = "yandex"`) sits alongside `JamendoProvider` and `AudiusProvider`. File name carries the `_unofficial` suffix and the module docstring states the ToS-violation honestly — see `requirements_ai.md` § 5a (Unofficial Provider Honesty Rule). Wraps the community `yandex-music==2.5.0` Python library; `__init__` lazy-imports it and raises `ImproperlyConfigured` if the package or `YANDEX_MUSIC_TOKEN` env var is missing.
- `Source.YANDEX = "yandex"` added to `catalog.models.Source.choices`; migration `catalog/0003_alter_source_choices_yandex.py` runs an `AlterField` on `Artist`/`Album`/`Track.source`. `CatalogSyncService.get_provider` instantiates the Yandex client on demand.
- Yandex stream URLs are short-lived (signed MD5 path, ~60 s TTL). New constant `SHORT_LIVED_AUDIO_SOURCES = frozenset({Source.YANDEX})`; `resolve_stream_url` bypasses the `audio_url_cached`/`audio_url_cached_at` flow for sources in that set and returns a fresh URL on every request. The existing `playback.api.views.StreamView` range-proxy works unchanged because the URL travels in query params, not auth headers.
- `DEFAULT_CATALOG_SOURCE` env var (defaults to `"yandex"` when `YANDEX_MUSIC_TOKEN` is set, else `"jamendo"`) drives the `_resolve_source` helper in `catalog.api.views`. `PopularTracksView`, `NewReleasesView`, `GenreTrackingView`, `RecommendationsView` all accept `?source=jamendo|audius|yandex` and fall back to the configured default; cache keys now include the source so caches don't collide between providers.
- New `.env.example` keys `YANDEX_MUSIC_TOKEN=` (labeled "UNOFFICIAL, ToS violation, dev/personal only") and `DEFAULT_CATALOG_SOURCE=jamendo`. Settings read both into `settings.YANDEX_MUSIC_TOKEN` / `settings.DEFAULT_CATALOG_SOURCE`. Personal token: extract from `music.yandex.ru` browser session (see `yandex-music` library docs).
- Frontend honesty: new `<SourceBadge source={track.source} />` in `app/shared/ui/source-badge/` renders a pill with the provider name (Jamendo / Audius / Yandex Music) under the artist name in every `<TrackRow>`. Yandex variant gets the accent green border so users always know which tracks are coming from the unofficial source. New i18n keys `common.source.{aria-label,jamendo,audius,yandex}` in en/ru/uz `common.json`.
- PWA workbox cache extended: cover images from `mds.yandex.net` / `avatars.yandex.net` / `avatars.mds.yandex.net` now match the same `CacheFirst` rule as `usercontent.jamendo.com` / `audius.co` (7-day TTL, max 200 entries).
- `plan.md` § 1 + new § 1a relax the original "we won't ship a project that breaks ToS" rule with an explicit, scoped exception for this single provider; `requirements_ai.md` § 5a defines the honesty rule that applies if any future unofficial provider is added.
